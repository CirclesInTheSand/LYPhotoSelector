...
- (void)showImage
{
    if (_photo.firstShow) { // 首次显示,开始下载咯
...
- (void)adjustFrame
{
	if (_imageView.image == nil) return;//imageView有image时才调整frame
    ...
    CGFloat imageWidth = imageSize.width; /**< 图片真实宽度 */
    CGFloat imageHeight = imageSize.height; /**< 图片真实高度 */
	...
	if ([UIScreen instancesRespondToSelector:@selector(scale)]) {
		maxScale = maxScale / [[UIScreen mainScreen] scale];  //scale == 2 || scale == 3
        //maxScale == 1 || maxScale == 0.666666 最大1倍或者...0.66倍
	}
    ...
    if (_photo.firstShow) { // 第一次显示的图片
        ...
        CGRect convertedRect = [_photo.srcImageView convertRect:_photo.srcImageView.superview.bounds toView:nil];
        _imageView.frame = convertedRect;
